...
void taskI2SOutput(void *param)
{
    ...
    while (true)
    {
        ...
        i2s_event_t evt;
        if (xQueueReceive(output->m_i2sOutputQueue, &evt, portMAX_DELAY) == pdPASS)
        {
            if (evt.type == I2S_EVENT_TX_DONE)
            {
                ...
                do
                {
                    if (availableBytes == 0)
                    {
                        if (!output->m_i2sOutputSampleGenerator->getFrames(frames, NUM_FRAMES_TO_SEND))
                        {
                            // No more frames were filled, end of playback by deleting this task
                            Serial.println("[taskI2SOutput] stopping");
                            output->stop();
                            Serial.println("[taskI2SOutput] stopped. YOU SHOULD NOT BE SEEING THIS MESSAGE");
                            return;
                        }
                    ...
                    if (availableBytes > 0)
                    {
                        ...
                        i2s_write(output->m_i2sOutputPort, buffer_position + (uint8_t *)frames, availableBytes, &bytesWritten, portMAX_DELAY);
...
void I2SOutput::start(i2s_port_t i2sOutputPort, i2s_pin_config_t &i2sOutputPins, SampleSource *i2sOutputSampleGenerator)
{
    m_i2sOutputSampleGenerator = i2sOutputSampleGenerator;
    // i2s config for writing both channels of I2S
    i2s_config_t i2sConfig = {
        .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_TX),
        .sample_rate = m_i2sOutputSampleGenerator->sampleRate(),
        ...
        .dma_buf_count = 4,
        .dma_buf_len = 1024};
    m_i2sOutputPort = i2sOutputPort;
    // install and start i2s driver
    i2s_driver_install(m_i2sOutputPort, &i2sConfig, 4, &m_i2sOutputQueue);
    // set up the i2s pins
    i2s_set_pin(m_i2sOutputPort, &i2sOutputPins);
    // clear the DMA buffers
    i2s_zero_dma_buffer(m_i2sOutputPort);
    // start a task to write samples to the i2s peripheral
    TaskHandle_t writerTaskHandle;
    Serial.println("[I2SOutput::start] creating task: 'i2s Writer Task'");
    xTaskCreate(taskI2SOutput, "i2s Writer Task", 4096, this, 1, &writerTaskHandle);
}
...
void I2SOutput::stop()
{
    ...
    vTaskDelete(m_taskI2SOutputHandle);
